...
def extract_data(data_dir):
    ...
    with open(data_path.replace('images', 'images.txt'), 'r') as f:
        for line in f:
            ...
            id_to_path_map[items[0]] = join(data_path, items[1])
    ...
    with open(join(cwd, data_dir + '/train_test_split.txt'), 'r') as f:
        for line in f:
            ...
            if int(is_train):
                train_val_id.append(idx)
    print("Number of train images from official train test split:", sum(list(is_train_test.values())))

    
    val_id=random.sample(train_val_id, int(len(train_val_id)/5))

    val_files=[]
    
    for i in val_id:
        val_files.append(id_to_path_map[i])
    ...
    for i, folder in enumerate(folder_list):
        ...
        for cf in classfile_list:
            ...
            metadata = {'id': img_id, 'img_path': img_path, 'class_label': i,
                      ...
                      'uncertain_attribute_label': attribute_uncertain_labels_all[img_id], 'original_attribute_label': attribute_labels_all[img_id]}
...
if __name__ == "__main__":
    ...
    for dataset in ['train','val','test']:
        ...
        f = open(join(os.getcwd(), args.save_dir, dataset + '.pkl'), 'wb+')
        if 'train' in dataset:
