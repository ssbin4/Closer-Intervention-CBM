"""
Train InceptionV3 Network using the SKINCON-200-2011 dataset
"""
import pdb
import os
import sys
...
from SKINCON import tti
from SKINCON.dataset import load_data, find_class_imbalance
from SKINCON.config import BASE_DIR, N_CLASSES, N_ATTRIBUTES, MIN_LR, LR_DECAY_SIZE, CLASS_RATIO, N_Y_OUTPUTS
from SKINCON.backbone import ModelXtoCY, ModelXtoChat_ChatToY, ModelXtoY, ModelXtoC, ModelOracleCtoY, ModelXtoCtoY
...
def run_epoch_simple(model, optimizer, loader, loss_meter, acc_meter, criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        ...
        _, class_preds = outputs.topk(1, 1, True, True)
        acc_meter.update(acc[0], inputs.size(0))

        all_preds.extend(list(class_preds.detach().cpu().numpy().flatten()))
        all_labels.extend(list(labels.flatten().data.cpu().numpy()))
    ...
    acc_list = []
    for i in range(N_CLASSES[args.class_label]):
        class_acc = np.mean(all_preds[all_labels == i]== i)
        acc_list.append(class_acc)
    print("class acc: ", acc_list)
    mean_accuracy = np.mean(np.array(acc_list))
    return loss_meter, acc_meter, f1, mean_accuracy
...
def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        ...
        if args.bottleneck: #attribute accuracy
            ...
            attr_preds = sigmoid_outputs.squeeze().cpu() >= 0.5
            all_labels.extend(list(attr_labels.flatten().data.cpu().numpy()))
            all_preds.extend(list(attr_preds.flatten().data.cpu().numpy()))
        else:
            acc = accuracy(outputs[0], labels, topk=(1,)) #only care about class prediction accuracy
            acc_meter.update(acc[0], inputs.size(0))
            _, class_preds = outputs[0].topk(1, 1, True, True)
    ...
    return loss_meter, acc_meter, f1, balanced_acc

def train(model, args):
    # Determine imbalance
    imbalance = None
    if args.use_attr and not args.no_img and args.weighted_loss:
        train_data_path = os.path.join(BASE_DIR, args.data_dir, args.class_label, 'train.pkl')
    ...
    if args.use_class_weights:
        class_imbalance = 1/np.array(CLASS_RATIO[args.class_label])
        criterion = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor(class_imbalance).cuda())
    else:
    ...
    if args.ckpt: #retraining
        train_loader = load_data([train_data_path, val_data_path], args.use_attr, args.no_img, args.batch_size, args.uncertain_labels, image_dir=args.image_dir, \
                                 n_class_attr=args.n_class_attr, resampling=args.resampling, class_label=args.class_label)
        val_loader = None
    else:
        train_loader = load_data([train_data_path], args.use_attr, args.no_img, args.batch_size, args.uncertain_labels, image_dir=args.image_dir, \
                                 n_class_attr=args.n_class_attr, resampling=args.resampling, class_label=args.class_label)
        val_loader = load_data([val_data_path], args.use_attr, args.no_img, args.batch_size, image_dir=args.image_dir, n_class_attr=args.n_class_attr, class_label=args.class_label)
    ...
    with torch.no_grad():
        if not args.ckpt: # evaluate on val set
            ...
            with torch.no_grad():
                if args.no_img:
                    val_loss_meter, val_acc_meter, val_f1, val_avg_acc = run_epoch_simple(model, optimizer, val_loader, val_loss_meter, val_acc_meter, criterion, args, is_training=False)
                else:
                    val_loss_meter, val_acc_meter, val_f1, val_avg_acc = run_epoch(model, optimizer, val_loader, val_loss_meter, val_acc_meter, criterion, attr_criterion, args, is_training=False)
            val_loss_avg = val_loss_meter.avg
            
            logger.write('Epoch [%d]:\t'
                    'Val loss: %.4f\tVal acc: %.4f\tVal f1: %.4f\tVal avg acc: %.4f\t'
                    'Best val epoch: %d\n'
                    % (0, val_loss_avg, val_acc_meter.avg, val_f1, val_avg_acc, best_val_epoch)) 
            logger.flush()
    ...
    for epoch in range(0, args.epochs):
        ...
        if args.no_img:
            train_loss_meter, train_acc_meter, train_f1, train_avg_acc = run_epoch_simple(model, optimizer, train_loader, train_loss_meter, train_acc_meter, criterion, args, is_training=True)
        else:
            train_loss_meter, train_acc_meter, train_f1, train_avg_acc = run_epoch(model, optimizer, train_loader, train_loss_meter, train_acc_meter, criterion, attr_criterion, args, is_training=True)
        ...
        if not args.ckpt: # evaluate on val set
            ...
            val_f1 = train_f1
            val_avg_acc = train_avg_acc
    

        if best_val_acc < val_avg_acc:
        ...
        logger.write('Epoch [%d]:\tTrain loss: %.4f\tTrain accuracy: %.4f\tTrain f1: %.4f\tTrain avg acc: %.4f\t'
                'Val loss: %.4f\tVal acc: %.4f\tVal f1: %.4f\tVal avg acc: %.4f\t'
                'Best val epoch: %d\n'
                % (epoch, train_loss_avg, train_acc_meter.avg, train_f1, train_avg_acc, val_loss_avg, val_acc_meter.avg, val_f1, val_avg_acc,best_val_epoch)) 
        logger.flush()
        ...
        if epoch >= 100 and val_avg_acc < 0.03:
...
def train_X_to_C(args):
    model = ModelXtoC(pretrained=args.pretrained, freeze=args.freeze, num_classes=N_Y_OUTPUTS[args.class_label], use_aux=args.use_aux,
...
def train_oracle_C_to_y_and_test_on_Chat(args):
    num_classes = N_Y_OUTPUTS[args.class_label]
    model = ModelOracleCtoY(n_class_attr=args.n_class_attr, n_attributes=args.n_attributes,
                            num_classes=num_classes, expand_dim=args.expand_dim)
    train(model, args)
...
def train_X_to_C_to_y(args):
    ...
    model = ModelXtoCtoY(n_class_attr=args.n_class_attr, pretrained=args.pretrained, freeze=args.freeze,
                         num_classes=num_classes, use_aux=args.use_aux, n_attributes=args.n_attributes,
                         expand_dim=args.expand_dim, use_relu=args.use_relu, use_sigmoid=args.use_sigmoid)
    train(model, args)

def train_X_to_y(args):
    model = ModelXtoY(pretrained=args.pretrained, freeze=args.freeze, num_classes=N_Y_OUTPUTS[args.class_label], use_aux=args.use_aux)
    train(model, args)

def train_X_to_Cy(args):
    model = ModelXtoCY(pretrained=args.pretrained, freeze=args.freeze, num_classes=N_Y_OUTPUTS[args.class_label], use_aux=args.use_aux,
...
def parse_arguments(experiment):
    ...
    parser = argparse.ArgumentParser(description='SKINCON Training')
    ...
    if experiment == 'TTI':
        ...
        parser.add_argument('-use_class_weights', action='store_true', help='whether to use weighted loss for class labels')
