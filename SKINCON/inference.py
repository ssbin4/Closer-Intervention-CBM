...
from SKINCON.dataset import load_data
from SKINCON.config import BASE_DIR, N_CLASSES, N_ATTRIBUTES
from analysis import AverageMeter, multiclass_metric, accuracy, binary_accuracy

K = [1] #top k class accuracies to compute
...
def eval(args):
    ...
    all_img_paths: array of image path
    """
    if args.model_dir:
    ...
    data_dir = os.path.join(BASE_DIR, args.data_dir, args.class_label, args.eval_data + '.pkl')
    loader = load_data([data_dir], args.use_attr, args.no_img, args.batch_size, image_dir=args.image_dir,
                       n_class_attr=args.n_class_attr, return_path=True, class_label=args.class_label)
    ...
    incorrect_cnt = np.zeros(args.n_attributes)            # i-th element: number of images where i number of concepts are mispredicted
    ...
    for data_idx, data in enumerate(loader):
        ...
        "may need to define scaled version"
    ...
    if args.class_label != 'binary':
        ...
        topk_class_outputs = all_class_outputs
        wrong_idx = np.where(np.sum(all_class_outputs == all_class_labels) == 0)
    ...
    class_f1 = f1_score(all_class_labels, all_class_outputs, average='macro')
    class_balanced_acc = balanced_accuracy_score(all_class_labels, all_class_outputs)

    return class_acc_meter, attr_acc_meter, all_class_labels, topk_class_outputs, all_class_logits, all_attr_labels, all_attr_outputs, all_attr_outputs_sigmoid, wrong_idx, all_attr_outputs2, incorrect_idx_list, all_image_paths, yerr_by_cerr, class_f1, class_balanced_acc
...
if __name__ == '__main__':
    ...
    y_f1_results, y_balanced_acc_results = [], []
    for i, model_dir in enumerate(args.model_dirs):
        ...
        if attr_acc_meter is not None:
            ...
            c_results.append(-1)
        y_f1_results.append(result[-2])
        y_balanced_acc_results.append(result[-1])
    ...
    print("y_f1", np.array(y_f1_results).mean())
    print("y_balanced_acc", np.array(y_balanced_acc_results).mean())
