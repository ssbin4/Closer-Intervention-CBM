...
from SYNTHETIC.config import N_CLASSES, N_ATTRIBUTES
...
def simulate_intervention(intervention_order, criterion, replace_val, preds_by_attr, ptl_5, ptl_95, model2, b_attr_binary_outputs, b_class_labels, b_class_logits,
    ...
    return class_acc, b_attr_new.flatten()
...
def parse_arguments(parser=None):
    ...
    parser.add_argument('-criterion', help='Which mode to use for correction. Choose from wrong_idx, entropy, uncertainty, random', default='entropy')
...
def run(args):
    ...
    _, _, b_class_labels, b_topk_class_outputs, b_class_logits, b_attr_labels, b_attr_outputs, b_attr_outputs_sigmoid, \
        b_wrong_idx, b_attr_outputs2, incorrect_idx_list, img_path_list, yerr_by_cerr = eval(args)
    ...
    for trial in range(N_TRIALS):
        ...
        for n_replace in list(range(intervention_num)):
            acc, b_attr_intervened = simulate_intervention(intervention_order,
    ...
    return np.array([float(err) for err in err_mean_list])
